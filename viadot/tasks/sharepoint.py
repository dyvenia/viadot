...
class SharepointToDF(Task):
    ...
    def check_column_names(self, df_header, header_to_compare) -> List:
        """
        ...
        if header_to_compare is not None:
            if df_header_list != header_to_compare:
                logging("Columns in sheets are different")
                exit()
    ...
    def split_sheet(self, sheetname, skiprows, nrows, chunks, **kwargs) -> List:
        """
        ...
        while True:
            ...
            if not df_chunk.shape[0]:
                ...
                logging(f" - chunk {i_chunk+1} ({df_chunk.shape[0]} rows)")
    ...
    def run(
        ...
        **kwargs,
        ...
        s = Sharepoint()
        s.download_file(self.url_to_file, filename=filename)
