...
class SharepointList(Source):
    ...
    def get_connection(
        ...
        try:
            ...
            auth_context.with_client_certificate(tenant=self.credentials["TENANT"],
    ...
    def get_fields(
        ...
        if required_fields is None:
            ...
            list_fields_required = [list_fields_all.get_by_internal_name_or_title(field).get() 
    ...
    def select_expandable_user_fields(
        ...
        list_fields = self.get_fields(list_title=list_title,
                                      site_url=site_url,
                                      required_fields=required_fields)
        
        # Finding the "selected" fields
        fields_to_select = [field.properties['InternalName'] +f"/{field_property}" 
                            if fnmatch(field.properties['TypeAsString'],"User*")  
                            else field.properties['InternalName'] for field in list_fields]
        # Finding the "expanded" fields
        fields_to_expand = [field.properties['InternalName'] for field in list_fields 
                            if fnmatch(field.properties['TypeAsString'],f"User*")]
        
        # Creating the body of the function output
        selected_fields = {"FieldInternalNames" : fields_to_select, "FieldToExpand": fields_to_expand, "FieldProperty": field_property}
    ...
    def check_filters(
        ...
        for parameters in filters.values():
                if parameters.get('dtype') not in allowed_dtypes:
                    raise ValueError(f"dtype not allowed! Expected {allowed_dtypes} got: {parameters.get('dtype')}.")
                if parameters.get('operator1'):
                    if parameters.get('operator1') not in allowed_operators:
                        raise ValueError(f"Operator type not allowed! Expected {allowed_operators} got: {parameters.get('operator1')}.")
                ...
                if parameters.get('operator2'):
                    ...
                    if not parameters.get('operators_conjuction'):
                        raise ValueError(f"Operators for conjuction is missing! Expected {allowed_conjuction} got empty.")
    ...
    def operators_mapping(
        ...
        for parameters in filters_dict.values():
            if parameters.get('operator1'):
                ...
                else: raise ValueError(f'This comparison operator: {operator1_to_change} is not allowed. Please read the function documentation for details!')
            if parameters.get('operator2'):
                ...
                else: raise ValueError(f'This comparison operator: {operator2_to_change} is not allowed. Please read the function documentation for details!')
            if parameters.get('operators_conjuction'):
                ...
                if logical_op_to_change in logical_op.keys():
                    parameters['operators_conjuction'] = logical_op[logical_op_to_change]
                else: raise ValueError(f'This conjuction(logical) operator: {logical_op_to_change} is not allowed. Please read the function documentation for details!')
            if parameters.get('filters_conjuction'):
                ...
                else: raise ValueError(f'This conjuction(logical) operator: {logical_fl_to_change} is not allowed. Please read the function documentation for details!')
        
        return filters_dict


    def make_filter_for_api(
            self,
            filters: dict
            ) -> 'str':
        """
        ...
        for column, parameters in filters_mod.items():
            if parameters.get('dtype') in ['datetime','date']:
                from_date1 = datetime.strptime(parameters.get('value1'),'%Y-%m-%d').isoformat()
                filter_text = filter_text + f"{column} {parameters.get('operator1')} datetime'{from_date1}' "
                if parameters.get('operator2'):
                    from_date2 = datetime.strptime(parameters.get('value2'),'%Y-%m-%d').isoformat()
                    filter_text = filter_text + f" {parameters.get('operators_conjuction')} {column} {parameters.get('operator2')} datetime'{from_date2}' "
            elif parameters.get('dtype') not in ['datetime','date']:
                filter_text = filter_text + f"{column} {parameters.get('operator1')} '{parameters.get('value1')}'"
                if parameters.get('operator2'):
                    filter_text = filter_text + f"{column} {parameters.get('operator2')} '{parameters.get('value2')}'"
            if parameters.get('filters_conjuction'):
    ...
    def make_filter_for_df(
        ...
        for column, parameters in filters.items():
                filter_in_df = filter_in_df + f"(df.{column} {parameters.get('operator1', '')} '{parameters.get('value1', '')}'"

                if parameters.get('operator2'):
                    filter_in_df = filter_in_df + f") {parameters.get('operators_conjuction')} (df.{column} {parameters.get('operator2', '')} '{parameters.get('value2', '')}'"
    ...
    def list_item_to_df(
        ...
        if filters is not None:
            ...
            for key in filters:
                if key not in required_fields:
                    raise AttributeError(f"Filter '{key}' not included inside required fields. It is obligatory to extract data which is filtered!")
        ...
        selected_fields = self.select_expandable_user_fields(list_title=list_title,
                                                             site_url=site_url,
                                                             required_fields=required_fields,
        ...
        try:
            ...
            if filters is None:
                ...
                list_items= self.list_object.items.filter(filter_text).select(selected_fields["FieldInternalNames"]).top(row_count).expand(selected_fields["FieldToExpand"])
            ...
            list_items = self.list_object.items.get_all(row_count,log_of_progress).select(selected_fields["FieldInternalNames"]).expand(selected_fields["FieldToExpand"])
            self.ctx.load(list_items)
            self.ctx.execute_query()
            download_all = True

        df = pd.DataFrame([self._unpack_fields(row_item,selected_fields) for row_item in list_items])
