...
class SalesForce(Source):
    ...
    Args:
        domain (str, optional): Domain of a connection. Defaults to 'test' (sandbox).
            Can only be add if a username/password/security token is provide.
        ...
        credentials (Dict[str, Any], optional): Credentials to connect with 
            Salesforce. If not provided, will read from local config file. 
            Defaults to None.
    ...
    def upsert(
        ...
        """
        Upsert the DataFrame to Salesforce. The upsert is performed on a single record
        at a time. Using an upsert operation gives more control over logging and error
        handling than using bulk upsert.

        Args:
            df (pd.DataFrame): Pandas DataFrame specified the rows to upsert.
            table (str): The table where the data should be upserte.
            external_id_column (str, optional): The external ID to use for the upsert. 
                Defaults to None.
        ...
        for record in records_cp:
            ...
            if external_id_column:
                # If the specified external ID is on the upsert line and has a value, it will 
                # be used as merge_key
    ...
    def bulk_upsert(
        ...
        Bulk upsert is performed on multiple records simultaneously, it is usually 
        used when there is a need to insert or update multiple records in a single 
        transaction, which can be more efficient and reduce the number of API calls 
        required.
        ...
        Args:
            ...
            external_id_column (str, optional): The external ID to use for the upsert. 
            Defaults to None.
            batch_size (int, optional): Number of records to be included in each batch
                of records that are sent to the Salesforce API for processing. 
                Defaults to 10000.
