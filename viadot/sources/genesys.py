...
class GenesysCredentials(BaseModel):
    CLIENT_ID: str  # Databricks Organization ID
    CLIENT_SECRET: SecretStr  # The host address of the Databricks cluster.
    ENVIRONMENT: Optional[str]  # The host address of the Genesys cluster.
    SCHEDULE_ID: Optional[str]  # The ID of the report.
    IDS_MAPPING: Optional[Dict[str, Any]]  # The mapping of IDs to strings.
...
class Genesys(Source):
    def __init__(
        ...
        credentials: GenesysCredentials = None,
        config_key: str = None,
        ...
        **kwargs: Dict[str, Any],
        ...
        Args:
            ...
            credentials (GenesysCredentials, optional): Credentials to connect with Genesys API.
        ...
        Raises:
            CredentialError: If credentials are not available.
        """

        credentials = credentials or get_source_credentials(config_key)
        if credentials is None:
            raise CredentialError("Please specify the credentials.")
        GenesysCredentials(**credentials)  # validate the credentials schema
    ...
    def download_all_reporting_exports(
        ...
        """
        Get information from data report and download all files.
        ...
        for report in self.report_data:
            ...
            if report[-1] == "RUNNING":
                ...
                continue
            elif report[-1] == "FAILED":
                self.logger.warning(
                    "This message 'FAILED_GETTING_DATA_FROM_SERVICE' raised during script execution."
                )
                continue
            elif self.start_date not in report[5]:
                self.logger.warning(
                    f"The report with ID {report[0]} doesn't match with the interval date that you have already defined. \
                        The report won't be downloaded but will be deleted."
                )
                continue

            if report[4].lower() == "queue_performance_detail_view":
                file_name = (temp_ids_mapping.get(report[2]) + "_" + report[3]).upper()
            elif report[4].lower() in [
        ...
        self.logger.info("Al reports were successfully download.")

        if store_file_names is True:
            self.logger.info("Successfully generated file names list.")
