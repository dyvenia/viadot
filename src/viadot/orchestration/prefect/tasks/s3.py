...
def s3_download_file(
    ...
    """Task to download a file from Amazon S3.

    Args:
        from_path (str): Path to the Amazon S3 file/folder.
        to_path (str): Path to the local file/folder where the downloaded file(s) will
            be saved.
        credentials (dict[str, Any], optional): Credentials to the Amazon S3.
            Defaults to None.
        config_key (str, optional): The key in the viadot config holding relevant
    ...
    Example:
        ...
        def test_flow():
            s3_download_file(
                from_path='s3://bucket_name/test.parquet',
                to_path="test.parquet",
                credentials= {
                    'profile_name': 'your_profile'
                    'region_name': 'your_region'
                    'aws_access_key_id': 'your_access_key_id'
    ...
    s3.download(from_path=from_path, to_path=to_path)
