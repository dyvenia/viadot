...
@flow(
    name="Customer Gauge extraction to ADLS",
    description="Extract data from Customer Gauge and load it into Azure Data Lake Storage.",
    retries=1,
    retry_delay_seconds=60,
    task_runner=ConcurrentTaskRunner,
)
def customer_gauge_to_adls(
    customer_gauge_config_key: str | None = None,
    customer_gauge_credentials_secret: str | None = None,
    ...
    Args:
        cutomer_gauge_config_key (str, optional): The key in the viadot config holding relevant
            credentials. Defaults to None.
        cutomer_gauge_credentials_secret (str, optional): The name of the Azure Key Vault secret
    ...
    data_frame = customer_gauge_to_df(
        config_key=customer_gauge_config_key,
        azure_key_vault_secret=customer_gauge_credentials_secret,
        endpoint=endpoint,
        cursor=cursor,
        pagesize=pagesize,
...
if __name__=="__main__":
    customer_gauge_to_adls(
        columns_to_anonymize= [
            ...
            "phone",
            "link",
            "properties_postal_code",
            "properties_city",
        ],
        unpack_by_field_reference_cols= ["properties"],
        endpoint= "non-responses",
        anonymize= True,
        total_load= True,
        date_column= "date_creation",
        days= 2 * 365,
        customer_gauge_credentials_secret= "customer-gauge",
        adls_credentials_secret= "app-azure-cr-datalakegen2-dev",
        adls_path= "raw/customer_gauge/customer_gauge_non_responses_anonymized.parquet",
    )