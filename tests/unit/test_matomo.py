...
class TestMatomoCredentials:
    """Test MatomoCredentials class."""
...
class TestMatomoInit:
    """Test Matomo initialization."""

    @patch("viadot.sources.matomo.get_source_credentials")
    def test_init_with_credentials_dict(self, mock_get_creds, matomo_credentials):
    ...
    @patch("viadot.sources.matomo.get_source_credentials")
    def test_init_with_config_key(self, mock_get_creds, matomo_credentials):
    ...
    def test_init_sets_data_to_none(self, matomo_instance):
        """Test that data attribute is initialized to None."""
        assert matomo_instance.data is None

    @patch("viadot.sources.matomo.get_source_credentials")
    def test_init_with_matomo_credentials_object(self, mock_get_creds):
...
class TestMatomoFetchData:
    """Test Matomo fetch_data method."""

    @patch("viadot.sources.matomo.handle_api_response")
    def test_fetch_data_success(
        self, mock_handle_api_response, matomo_instance, sample_matomo_response
    ):
    ...
    def test_fetch_data_with_empty_params_raises_validation_error(
        self, matomo_instance
    ):
    ...
    def test_fetch_data_with_none_params_raises_validation_error(self, matomo_instance):
        """Test fetch_data with None params raises validation error."""
        with pytest.raises(ValueError, match="Missing required API parameters"):
            matomo_instance.fetch_data("token", "https://example.com", None)

    @patch("viadot.sources.matomo.handle_api_response")
    def test_fetch_data_http_error(self, mock_handle_api_response, matomo_instance):
    ...
    def test_fetch_data_request_exception(
        self, mock_handle_api_response, matomo_instance
    ):
    ...
    def test_fetch_data_missing_api_token(self, matomo_instance):
        """Test fetch_data raises error when api_token is empty."""
        with pytest.raises(
            ValueError, match="api_token is required and cannot be empty"
        ):
            matomo_instance.fetch_data("", "https://example.com", {"module": "API"})

    def test_fetch_data_missing_url(self, matomo_instance):
    ...
    def test_fetch_data_missing_required_params(self, matomo_instance):
        ...
        with pytest.raises(ValueError, match="Missing required API parameters"):
            matomo_instance.fetch_data(
                "token", "https://example.com", incomplete_params
            )

    @patch("viadot.sources.matomo.handle_api_response")
    def test_fetch_data_invalid_json_response(
        self, mock_handle_api_response, matomo_instance
    ):
...
class TestMatomoToDf:
    """Test Matomo to_df method."""

    def test_to_df_without_data_raises_error(self, matomo_instance):
    ...
    def test_to_df_basic_conversion(self, matomo_instance, sample_matomo_response):
    ...
    def test_to_df_with_record_prefix(self, matomo_instance, sample_matomo_response):
    ...
    def test_to_df_with_list_record_path(self, matomo_instance, sample_matomo_response):
    ...
    def test_to_df_empty_result_warn(self, matomo_instance):
        ...
        with patch("viadot.sources.base.logger.warning") as mock_warning:
            ...
            mock_warning.assert_any_call(
                "No records found in the specified record_path."
            )
            mock_warning.assert_any_call("The query produced no data.")

    def test_to_df_empty_result_skip(self, matomo_instance):
    ...
    def test_to_df_empty_result_fail(self, matomo_instance):
    ...
    def test_to_df_with_tests_validation(self, matomo_instance, sample_matomo_response):
...
class TestMatomoIntegration:
    """Test Matomo integration scenarios."""
    ...
    def test_data_persistence(self, matomo_instance, sample_matomo_response):
