...
def test_wrong_parameters_codes_logger(caplog):
    ...
    with pytest.raises(ValueError, match="Wrong parameters or codes were provided!"):
        with caplog.at_level(logging.ERROR):
            Eurostat(dataset_code=dataset_code).make_params_validation(
...
def task_correct_requested_columns(caplog):
    ...
    task = Eurostat(dataset_code="ILC_DI04").requested_columns_validation(
        data_frame=df, columns=columns
    )
