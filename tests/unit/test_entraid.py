...
async def test_get_access_token_success():
    ...
    session = MockSession(post_response=MockResponse(status=200, json_data={"access_token": "tok"}))
    token = await e._get_access_token(session)  # type: ignore[arg-type]
    assert token == "tok"
...
@pytest.mark.asyncio
async def test_fetch_all_pages_pagination_with_retry(monkeypatch):
    e = EntraID(credentials=_base_credentials())
    # 1st call -> 429 with Retry-After=0, then 2 pages 200
    r1 = MockResponse(status=429, headers={"Retry-After": "0"})
    r2 = MockResponse(status=200, json_data={"value": [{"a": 1}], "@odata.nextLink": "next"})
...
async def test_get_all_users_calls_fetch_with_url(monkeypatch):
    ...
    async def fake_fetch(session, url, *args, **kwargs):
        captured["url"] = url
...
async def test_build_user_groups_df_async_aggregates(monkeypatch):
    ...
    async def fake_get_groups(session, user):
        if user["id"] == "1":
    ...
    users_df = pd.DataFrame([{"id": "1", "mail": "a@b.com"}, {"id": "2", "mail": "c@d.com"}])
    session = MagicMock()
    df = await e._build_user_groups_df_async(session, users_df, ["user_email", "group_name"], max_concurrent=5)  # type: ignore[arg-type]
...
async def test_build_users_groups_df_composes(monkeypatch):
    ...
    async def fake_authorize(session):
        return None

    async def fake_get_users(session):
        return [{"id": "1", "mail": "a@b.com"}]

    async def fake_build(session, users, output_fields, max_concurrent):
