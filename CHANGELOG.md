...
### Added
- Sources:
Added connector file `outlook.py` in source.
Added test file `test_outlook.py`.
...
- Added `Outlook` source to library its `test` file non-existing previously.
...
- Changed `to_df` method in `Outlook` source file to be more readable.
